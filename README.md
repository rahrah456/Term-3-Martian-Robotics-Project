...
The robot requires calibration of its encoder-based motion to achieve accurate dead-reckoning, and its magnetometer for reliable absolute heading.

### Magnetometer Calibration

Magnetometers are highly sensitive to their surrounding environment and the frame they are mounted on. Without calibration, raw readings often yield erratic, skewed headings due to two specific types of magnetic interference:

* **Hard-Iron Distortion:** Caused by constant magnetic fields generated by nearby components (e.g., batteries, metal chassis, or structural steel beams). This shifts the entire 3D data cluster away from the true $(0,0,0)$ origin.
* **Soft-Iron Distortion:** Caused by materials like iron or steel that dynamically alter and warp the surrounding geomagnetic field. This squishes what should be a perfect spherical data cluster into an elongated ellipsoid.

#### Linear Transformation Strategy

By performing a 3D ellipsoid fit on the collected data points, this script calculates a **3D Offset Vector (Hard-Iron Bias)** and a **3x3 Inverse Scaling Matrix (Soft-Iron Bias)**.

When applied to live sensor tracking, the algorithm shifts the offset data back to centre and transforms the squished ellipsoid back into a uniform sphere. This process scales raw sensor units into physically accurate microTeslas ($\mu\text{T}$), enabling precise, tilt-compensated heading calculations.

#### Raw Readings (Uncalibrated)

The raw magnetometer output forms a cluster offset from the origin, with each axis showing different ranges — clear evidence of hard-iron distortion on a dominant axis.

![Raw magnetometer readings showing the hard-iron offset](README_resources/Figure_1.png)

#### Ellipsoid Fit

A 3D ellipsoid is fitted to the collected raw samples. The elongation and rotation of this ellipsoid relative to a perfect sphere quantifies the soft-iron distortion, while its centre gives the hard-iron offset vector.

![Ellipsoid fit of the raw magnetometer data](README_resources/Figure_2.png)

#### Corrected Spherical Data

After applying both the hard-iron offset and the soft-iron inverse scaling matrix, the data collapses onto a well-centred sphere of radius equal to the local geomagnetic field strength (~49 $\mu\text{T}$).

![Corrected data forming a centred sphere](README_resources/Figure_3.png)

#### Tilt-Compensated Heading

The calibrated magnetometer vectors are combined with accelerometer pitch/roll data from the LSM6 to project a stable, tilt-compensated heading that remains accurate even when the robot is on slopes.

![Tilt-compensated heading output](README_resources/Figure_4.png)

#### Calibration Parameters

The ellipsoid fit produced the following hard-iron bias vector (applied to $\mu\text{T}$-scaled readings) and soft-iron inverse scaling matrix:

```
Hard-iron bias (µT):    x = -92.43    y = 74.12     z = -1.23

Soft-iron matrix:
    [ 0.9288    0.0237    0.0705 ]
    [ 0.0237    0.9253   -0.0205 ]
    [ 0.0705   -0.0205    0.7979 ]
```

These parameters are hard-coded in `archive/component_tests/magnetometer_spherical_fitted_heading/magnetometer_spherical_fitted_heading.ino` and should be ported into the main sketch's `Sensors.h` when integrating the magnetometer back into the heading pipeline.
