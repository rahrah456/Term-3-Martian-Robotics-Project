...
Raw magnetometer readings suffer from hard-iron offsets (nearby magnetic fields shifting the origin) and soft-iron distortion (ferrous materials squishing the sphere into an ellipsoid). A 3D ellipsoid fit yields a hard-iron offset vector and a soft-iron inverse scaling matrix that transform the ellipsoid back to a centred sphere.

![Raw offset cluster](README_resources/Figure_1.png)
![Ellipsoid fit](README_resources/Figure_2.png)
![Corrected sphere](README_resources/Figure_3.png)
![Tilt-compensated heading](README_resources/Figure_4.png)

**Parameters** (µT-scaled, from `archive/component_tests/magnetometer_spherical_fitted_heading/`):

```
Hard-iron bias:  x=-92.43  y=74.12   z=-1.23
Soft-iron matrix: [0.9288  0.0237  0.0705]
...
Send the robot forward 2 m using the tick calibration sketch (`d <ticks>`). The left and right tracks will accumulate different tick counts due to manufacturing and load differences. Compute the bias as:

```
BIAS_RIGHT = left_ticks / right_ticks
```

Apply this in `setMotors()`: `right = (int)(right * BIAS_RIGHT)`. Repeat the straight-line test until both tracks match.

Our robot: `BIAS_RIGHT = 1.047`.

### Move Calibration

Plot measured distance against commanded encoder ticks for forward trials (`d <ticks> [trials]` in the calibration sketch). Fit the curve to:

```
distance_mm = 4.44137 * ln(ticks + 1) + 0.273899 * ticks
```

Inverted for `ticksForDistance()`:

```
ticksForDistance(mm) = 34.21488 * exp(-0.0870233 * mm - 1.0) + 3.60454 * mm
```
...
Plot measured rotation angle against commanded encoder ticks for tank-turn trials (`a <ticks> [trials]` in the calibration sketch). Fit the linear relationship:

```
ticksForTurn(degrees) = 6.0061 * degrees
```
...
1. Run the `tick_calibration.ino` sketch at 115200 baud.
2. For **move**: send `d <ticks> [trials]` — the robot drives forward at speed 500, averages multiple trials. Measure actual distance travelled and record the tick count.
3. For **turn**: send `a <ticks> [trials]` — the robot tank-turns at speed 660 with burst acceleration (180000), averages multiple trials. Measure actual rotation angle and record the tick count.
4. Collect data across a range of small and large commands. Focus more points on short moves for better low-end accuracy.
5. Fit the curves above to the (commanded ticks, measured distance/angle) pairs and update `Config.h`.
6. Re-calibrate the motor bias first — move calibration assumes tracks are balanced.
